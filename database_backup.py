...
class Database:
    ...
    def get_calendar_cleaning_assignments(self, date_str: str) -> List[Dict]:
        """Get all cleaning assignments for a specific date"""
        conn = self.get_connection()
        ...
        if self.is_postgres:
            ...
            cursor.execute('''
                SELECT ca.*, ct.nombre as task_nombre, ct.area, ct.descripcion,
                       ct.dificultad, ct.tiempo_estimado,
                       a.nombre as member_name
                FROM cleaning_assignments ca
                JOIN cleaning_tasks ct ON ca.task_id = ct.id
                LEFT JOIN adults a ON ca.member_id = a.id AND ca.member_type = 'adulto'
                LEFT JOIN children c ON ca.member_id = c.id AND ca.member_type = 'niño'
                WHERE ca.fecha_especifica = %s OR (ca.fecha_especifica IS NULL AND ca.dia_semana = %s)
                ORDER BY ct.dificultad DESC, ct.area
            ''', (date_str, datetime.strptime(date_str, '%Y-%m-%d').strftime('%A').lower()))
        else:
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            ...
            cursor.execute('''
                SELECT ca.*, ct.nombre as task_nombre, ct.area, ct.descripcion,
                       ct.dificultad, ct.tiempo_estimado,
                       CASE 
                           WHEN ca.member_type = 'adulto' THEN a.nombre
                           ELSE c.nombre
                       END as member_name
                FROM cleaning_assignments ca
                JOIN cleaning_tasks ct ON ca.task_id = ct.id
                LEFT JOIN adults a ON ca.member_id = a.id AND ca.member_type = 'adulto'
                LEFT JOIN children c ON ca.member_id = c.id AND ca.member_type = 'niño'
                WHERE ca.fecha_especifica = ? OR (ca.fecha_especifica IS NULL AND ca.dia_semana = ?)
                ORDER BY ct.dificultad DESC, ct.area
            ''', (date_str, day_name))
    ...
    def save_cleaning_assignment(self, assignment: Dict) -> int:
        """Save a cleaning assignment with calendar support"""
        conn = self.get_connection()
        cursor = conn.cursor()
        ...
        if self.is_postgres:
            cursor.execute('''
                INSERT INTO cleaning_assignments (task_id, member_id, member_type, 
                    dia_semana, week_start, fecha_especifica, tipo_asignacion, 
                    semana_referencia, completado, notas)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                ON CONFLICT (task_id, member_id, week_start) 
                DO UPDATE SET dia_semana = EXCLUDED.dia_semana, 
                              member_type = EXCLUDED.member_type,
                              fecha_especifica = EXCLUDED.fecha_especifica,
            ...
            ''', (
                ...
                assignment.get('week_start'), assignment.get('fecha_especifica'),
                assignment.get('tipo_asignacion', 'semanal'),
                assignment.get('semana_referencia'), assignment.get('completado', False),
